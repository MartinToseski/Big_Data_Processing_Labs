...
def safe_json_request(url, retries=5, base_delay=1):
...
for city in cities:
    ...
    search_query = urllib.parse.quote(f'tourist attractions in {city}')
    ...
    for page in pages:
        try:
            ...
            links_data = safe_json_request(links_url)
            wiki_links = []
            if links_data:
                for value in links_data['query']['pages'].values():
                    ...
                    for link in value['links']:
                        wiki_links.append(link['title'])

            wiki_links_text = '|'.join(wiki_links)
            ...
            coord_data = safe_json_request(coord_url)
            ...
            images_data = safe_json_request(images_api)
            candidate_images = []
            ...
            if images_data:
                for value in images_data['query']['pages'].values():
                    ...
                    for image in value['images']:
                        ...
                        candidate_images.append(image_title)
            ...
            for candidate in candidate_images:
                try:
                    ...
                    image_info = safe_json_request(image_info_api)
                    ...
                    print(f'Valid image selected: {title}')
                    break
